...
`cases.jsonl` 是 harness 运行时生成的逐 case 调试产物，体积较大，不作为当前提交的归档文件。
...
当前实现的 sparse boost 不改 BM25 / sparse index 底层公式，而是在 sparse input 中把每个 must_have term 重复 3 次。
例如 `Apple 2023 revenue` 会变成近似 `Apple 2023 revenue Apple Apple Apple 2023 2023 2023`。
这个做法粗暴但可控：它提高词法偏好，同时避免 hard filter 直接把正确 evidence 杀掉。
```
...
|---|---:|---:|---:|---|
| dense-only | 0.800 | 1.000 | 0.900 | smoke baseline |
| sparse-only | 1.000 | 1.000 | 1.000 | smoke baseline |
| Python Weighted RRF | 1.000 | 1.000 | 1.000 | 当前可执行 |
| Qdrant RRF | - | - | - | planned_not_run |

这组 synthetic smoke 太小，dense/sparse/RRF 在 page@3 都能命中，但 page@1 仍能看出 dense-only 的排序不稳。
因此它只能作为策略形状检查，不能用来证明 fusion 优劣。
...
| current unit + candidate | 0.800 | 1.000 | 0.900 | 与 original query 接近，仍有一个 case 排第 2 |
...
```text
smoke_3m_capex_2018
original query、current unit 和 full plan summary 都让正确页排第 2。
