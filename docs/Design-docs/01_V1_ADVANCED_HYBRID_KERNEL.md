...
User Query
  ...
  -> Provider Router
       ...
       executable_providers = ["hybrid"] in V1
...
User Query
  ...
  -> ProviderRouter
       ...
       sql / graph -> skipped_non_executable trace
...
```text
Provider 不是 lane。
V1 executable provider 只有 hybrid；语义 provider vocabulary 包含 hybrid / sql / graph。
Dense / BM25 / table textual 是 TextHybridProvider 内部 lanes。
SQLProvider 和 GraphProvider 可以出现在语义 QueryPlan 中，但不会在 V1 runtime 执行。
```
...
V1 runtime 只执行 `hybrid`，并把不可执行 provider 编译成 `skipped_non_executable` trace。
...
因此 V1 的 semantic plan 可以保留 sql / graph 意图，但 actual execution 只执行 hybrid：
...
```text
1. ProviderRouter 把 graph / sql tasks 标为 skipped_non_executable。
2. ProviderRouter 只把 hybrid tasks 交给 TextHybridProvider。
...
如果 evidence 不足，V1 应返回保守答案，并在 trace 中显示 graph/sql 未执行以及 hybrid source evidence 是否不足，而不是假装 graph/sql 已经执行。
...
non-executable sql/graph providers 被 skipped trace 记录
...
3. V1 runtime executable_providers 只有 ["hybrid"]。
4. sql/graph tasks 是 valid plan、skipped_non_executable execution result。
...
13. Graph 被标注为 future candidate generator，不能直接当 VIP EvidenceBlock。
