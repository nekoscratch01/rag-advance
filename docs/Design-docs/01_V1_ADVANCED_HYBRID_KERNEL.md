...
> V1 边界：V1 baseline 只启用 `hybrid` provider。V3.0 后当前默认 runtime 已启用 GraphProvider walking skeleton；`sql` 仍是未来 V4 候选能力。
...
| Provider | 目标能力 | V1 状态 |
|---|---|---|
| `hybrid` | 文本证据检索。内部包含 dense、BM25、metric alias、section、table textual lanes。 | 已启用，是 V1 baseline provider。 |
| `sql` | 结构化表格、SQL、计算、cell provenance。 | 未启用，属于 V4 候选能力。 |
| `graph` | 实体关系、跨文档线索、community/global context。 | V3.0 walking skeleton 已默认可执行；只在 QueryPlan 选择 graph 时运行。 |
...
这条证据链足够稳之后，`graph` 已在 V3.0 作为 walking skeleton provider 接入；后续 `sql` 仍必须作为 provider 接入，而不是绕过 Evidence Kernel 自己生成答案。
...
User Query
  -> Query Orchestrator
     -> Provider Router
       ...
       V1 baseline executable_providers = ["hybrid"]
       current default executable_providers = ["hybrid", "graph"]
     -> Provider-local retrieval
       hybrid: dense + sparse + textual table lanes
       sql: structured table / calculation, future only
       graph: V3 walking skeleton, executed only when QueryPlan selects graph
...
User Query
  -> Query Orchestrator
  -> semantic QueryPlan / RetrievalUnit(provider="hybrid" | "sql" | "graph")
  -> RetrievalTask
  -> ProviderRouter
       hybrid -> TextHybridProvider
              ...
              -> Reranker
       graph -> V3.0 GraphProvider local/path
       sql -> skipped_non_executable trace
  -> Cross-provider evidence normalization
...
```text
Provider 不是 lane。
V1 baseline executable provider 只有 hybrid；V3.0 后默认 executable providers 是 hybrid / graph。
语义 provider vocabulary 包含 hybrid / sql / graph。
Dense / BM25 / table textual 是 TextHybridProvider 内部 lanes。
SQLProvider 和 GraphProvider 可以出现在语义 QueryPlan 中；当前 runtime 会执行 graph task，SQL 仍 skipped。
```
...
V1 的 planner 使用完整 provider ontology。Plan 是语义层，Execution 是 runtime 能力层。

```python
known_providers = ["hybrid", "sql", "graph"]
v1_baseline_executable_providers = ["hybrid"]
current_default_executable_providers = ["hybrid", "graph"]
```

`sql` / `graph` 出现在 plan 中是合法的；它们表示“理想证据获取方式”。
V1 baseline 只执行 `hybrid`；当前默认 runtime 可执行 `hybrid` 和 `graph`，但 graph 只有被 QueryPlan 选中时才运行。
不可执行 provider 仍会编译成 `skipped_non_executable` trace。

```python
ProviderName = str

# Runtime validates provider names against configured known providers and rejects
# reserved internal lanes such as dense/bm25/table as provider identities.
...
provider = "hybrid"      # new contract field
```
...
但 V1 baseline runtime 只有 `hybrid` provider，当前默认 runtime 也仍不执行 SQL。如果 prompt 只说“选择最合适的 provider”，模型会自然输出：
...
这在架构上看起来合理；在 V1 baseline 中 `sql` 与 `graph` 都不可执行，而当前默认 runtime 中只有 `sql` 仍不可执行。这就是 Planner Prompt Paradox：

```text
架构 vocabulary 比 runtime executable providers 更大。
如果把 runtime 能力反向塞进 prompt，LLM 会把本来属于 SQL / Graph 的语义意图伪装成 hybrid。
```
...
```text
Return the provider that best matches the user's evidence need.
Do not hide SQL-like or graph-like intent by forcing it into hybrid.
Runtime execution may skip non-executable providers.
```
...
provider_results:
  - provider: sql
    ...
    planned_text: "Apple Microsoft R&D 2021 2022 2023 sum comparison"
...
```text
不自动 hybrid_backfill。
不把 sql / graph 任务伪装成 hybrid。
只有 successful ProviderResult 进入 EvidencePack。
skipped provider 只进入 trace / diagnostics。
```
...
当前默认 runtime 已启用 `graph` walking skeleton；如果一个问题同时需要 textual evidence 和 graph relation context，应拆成两个 unit：
...
```text
1. 如果 LLM 输出 unknown provider，带错误原因重试。
2. 如果 LLM 输出 compound provider unit，要求拆成 single-provider units 后重试。
3. 如果 LLM 输出 known but non-executable provider，plan 合法，runtime 记录 skipped_non_executable。
4. fallback 仍生成 deterministic hybrid plan，因为 fallback 是无 LLM 时的保守兜底。
```

这条规则防止 runtime 能力污染 planner ontology，同时避免 unknown/compound provider 进入 runtime。

---

## 6. TextHybridProvider 内部执行

`TextHybridProvider` 是 hybrid provider 的实现。它内部可以有多条 lane，但对外只产出 `provider="hybrid"` 的 candidates。
...
`graph` 在 V1 baseline 中不启用；V3.0 后当前默认 runtime 已启用 GraphProvider walking skeleton。
这里讨论的是 graph 能力边界，不是质量提升声明。
...
V1 baseline executable providers 只有 hybrid；当前 V3.0 后默认 executable providers 是：
...
因此 semantic plan 可以保留 sql / graph 意图；当前默认 execution 会执行 graph task，SQL 仍然 skipped：
...
retrieval_units:
  ...
  - unit_id: u2
    provider: hybrid
    purpose: annual_report_source_text
...
```text
1. ProviderRouter 把 sql tasks 标为 skipped_non_executable。
2. ProviderRouter 把 hybrid tasks 交给 TextHybridProvider，把 graph tasks 交给 V3.0 GraphProvider。
3. TextHybridProvider 生成 dense_text = unit.text。
4. TextHybridProvider 生成 sparse_text = unit.text + should_terms + repeat(must_have_terms, 3)。
5. metadata_filter 限制 filing_type / document_ids，如果调用方提供了可执行 metadata。
6. Qdrant dense 和 Qdrant BM25 sparse 分别召回 child chunks。
7. Python Weighted RRF 或 Qdrant RRF 合并。
8. Reranker 选择最能支持 AAPL/MSFT/Vision Pro 比较的 source chunks。
9. Evidence Builder 扩展到 parent/page blocks。
10. Answer Generator 只能基于 successful ProviderResult 产生的 evidence 做比较。
```
...
如果 evidence 不足，V1 应返回保守答案，并在 trace 中显示 SQL skipped、graph executed/empty/not-planned 的真实状态，以及 hybrid source evidence 是否不足，而不是假装未产生的证据已经存在。
...
```text
planner 能表达 known providers
compound units 被 retry 或 fallback
non-executable sql provider 被 skipped trace 记录
...
```text
1. Query Orchestrator 使用 known_providers planner ontology。
2. QueryPlan 可以表达 provider="hybrid" / "sql" / "graph"。
3. V1 baseline executable_providers 只有 ["hybrid"]；当前默认 runtime executable_providers 是 ["hybrid", "graph"]。
4. sql tasks 是 valid plan、skipped_non_executable execution result；graph tasks 在 V3.0 runtime 中可执行。
5. skipped provider 只进入 trace / diagnostics，不进入 EvidencePack。
6. 默认不自动 hybrid_backfill。
7. compound unit 会被拆分重试。
8. TextHybridProvider 明确执行 dense_text = unit.text。
9. TextHybridProvider 明确执行 sparse_text = unit.text + should_terms + repeat(must_have_terms, 3)。
10. metadata_filter 能进入 Qdrant payload filter，并写入 trace。
11. must_have_terms 的默认行为和实验行为可通过 eval 区分。
12. Table lane 被标注为 serialized table text stopgap。
13. Graph 被标注为 V3.0 walking skeleton candidate generator，不能声明质量提升，也不能绕过 EvidenceBlock。
14. trace / eval / cache 都能复盘每一步。
```
...
```text
Atlas 建立了 hybrid-executable Evidence Kernel。
最终架构保留 hybrid / sql / graph 三个 provider 槽位。
V1 baseline 只启用 hybrid；当前默认 runtime 已启用 V3 GraphProvider walking skeleton。
所有新增 provider 都必须回到 Candidate / EvidenceBlock / EvidencePack / VerificationResult 合约。
```
