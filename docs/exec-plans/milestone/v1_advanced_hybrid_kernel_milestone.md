...
```text
pyproject package version 调整到 1.0.0
README quickstart 改为 V1 Evidence Kernel 示例问题
新增 samples/v1_evidence_kernel.md，避免 quickstart 继续以历史 V0 baseline 作为入口
OpenAI structured planner schema 收紧 metadata_filter，兼容 gpt-5-nano strict JSON schema
retrieve_candidates_with_plan 增加直接单测，确认 wrapper 只返回 candidate run，不构造 evidence_pack
API DI 的 dense / BM25 adapter helper 改为私有装配函数
legacy retriever shim 改为显式窄导出，避免 import * 扩大旧导出面
```
