        ...
        try:
            if hasattr(self.qdrant, "query_points"):
                ...
                if bm25_sparse_enabled(self.settings):
                    kwargs["using"] = self.settings.qdrant_dense_vector_name
                result = self.qdrant.query_points(**kwargs)
                return result.points
            legacy_query_vector = query_vector
            if bm25_sparse_enabled(self.settings):
                legacy_query_vector = models.NamedVector(
                    name=self.settings.qdrant_dense_vector_name,
                    vector=query_vector,
                )
            return self.qdrant.search(
                collection_name=self.settings.qdrant_collection,
                query_vector=legacy_query_vector,
                query_filter=qdrant_filter,
                limit=top_k,
                with_payload=True,
            )
...
def _candidate_to_evidence(candidate: Candidate, evidence_index: int) -> Evidence:
    retrieval_score = candidate.dense_score if candidate.dense_score is not None else 0.0
    return Evidence(
        ...
        rank=candidate.dense_rank or candidate.final_rank or evidence_index,
